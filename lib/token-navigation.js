...
import FuzzyView, {
	...
	searchStatus,
	isFocused
} from './token-navigation-fuzzy-view';
...
export default {
	...
	activate(state) {
		...
		this.store.subscribe('token-select-effect', state => {
			if (searchStatus(state) === 'SEARCH_SUCCESS') {
				state.editor && state.editor.setSelectedBufferRange(state.results[state.selectedToken]);
			}
		});
	...
	focus() {
		this.store.dispatch({ type: 'FOCUS' });
	},
