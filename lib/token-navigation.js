...
export default {
	...
	nextNotIgnoredToken(tokens, index, selectedRange, offset) {
		for (let i = index + offset; i < tokens.length; i++) {
	...
	previousNotIgnoredToken(tokens, index, selectedRange, offset) {
		for (let i = index - offset; i >= 0; i--) {
			const token = tokens[i];
			if (this.shouldIgnore(token, selectedRange)) {
				continue;
			}
	...
	multiNextToken() {
		...
		// Hack until vim-mode-plus fixes cursor bug
		const offset = editor.getSelectedText() === '' ? 0 : 1;
		...
		const ranges = cursors.map((cursor, index) =>
			this.nextToken(editor, tokens, cursor, selectedRanges[index], offset)
		).filter(x => x);
		if (!ranges.length) {
			return;
		}
	...
	multiPreviousToken() {
		...
		const selectedRanges = editor.getSelectedBufferRanges();
		const ranges = cursors.map((cursor, index) =>
			this.previousToken(editor, tokens, cursor, selectedRanges[index], offset)
		).filter(x => x);
		if (!ranges.length) {
			return;
		}
	...
	nextToken(editor, tokens, cursor, selectedRange, offset) {
		...
		for (let i = 0; i < tokens.length - 1; i++) {
			...
			if (isInside(token)) {
				const nextValidToken = this.nextNotIgnoredToken(tokens, i, selectedRange, offset);
				if (!nextValidToken) {
					return;
				}
			...
			if (isAfter(token) && isBefore(nextToken)) {
				const nextValidToken = this.nextNotIgnoredToken(tokens, i + 1, selectedRange, offset);
				if (!nextValidToken) {
					return;
				}
				return toAtomRange(nextValidToken.loc);
	...
	previousToken(editor, tokens, cursor, selectedRange, offset) {
		const { line, column } = toBabelPoint(cursor.getBufferPosition());
		const isInside = isInToken({ line, column });
		const isBefore = isBeforeToken({ line, column });
		const isAfter = isAfterToken({ line, column });
		...
		for (let i = 1; i < tokens.length; i++) {
			...
			if (isInside(token)) {
				const previousValidToken = this.previousNotIgnoredToken(tokens, i, selectedRange, offset);
			...
			if (isBefore(token) && isAfter(previousToken)) {
				const previousValidToken = this.previousNotIgnoredToken(tokens, i - 1, selectedRange, offset);
				if (!previousValidToken) {
					return;
				}
				return toAtomRange(previousValidToken.loc);
