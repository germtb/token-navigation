...
const reducers = (state, { type, payload }) => {
	if (type === 'SHOW') {
		...
		if (searchStatus(nextState) === 'SEARCH_SUCCESS') {
			selectToken(nextState.editor, payload.results[nextState.selectedToken]);
		}
		return nextState;
...
class FuzzyToken extends React.PureComponent {
	...
	componentDidMount() {
		this.searchEditor.classList.add('fuzzy-view');
		// this.searchEditor.add('editor fuzzy-view mini');
	...
	componentWillUnmount() {
		...
		this.searchEditor.removeEventListener('core:cancel', this.hide);
		this.searchEditor.removeEventListener('token-navigation:nextFuzzyToken', this.next);
		this.searchEditor.removeEventListener('token-navigation:previousFuzzyToken', this.previous);
		this.searchEditor.removeEventListener('token-navigation:allFuzzyToken', this.selectAll)
	}
	...
	render() {
		...
		return (
			<atom-panel-container ref={ element => {
				...
				<atom-panel style={{ padding: '5px 10px' }} ref={ element => {
					if (!element) {
						return;
					}
					element.classList.add('tool-panel');
					element.classList.add('panel-bottom');
					element.classList.add('bottom');
				}}>
