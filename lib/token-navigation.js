...
export default {
	...
	nextToken(editor, tokens, cursor, selectedRange) {
		...
		for (let i = 0; i < tokens.length - 1; i++) {
			...
			const { start: nextStart, end: nextEnd } = nextToken.loc;
		...
		if (nextToken) {
			return this.toAtomRange(nextToken.loc.start, nextToken.loc.end);
		}
	},
	...
	previousToken(editor, tokens, cursor, selectedRange) {
		...
		if (previousToken) {
			return this.toAtomRange(previousToken.loc.start, previousToken.loc.end);
		}
	},
