...
export default {
	...
	nextNotIgnoredToken(tokens, index) {
	...
	previousNotIgnoredToken(tokens, index) {
		for (let i = index; i < tokens.length; i--) {
			const token = tokens[i];
			if (this.shouldIgnore(token)) {
				continue;
	...
	nextToken() {
		...
		for (let i = 0; i < tokens.length - 1; i++) {
			...
			nextToken = this.nextNotIgnoredToken(tokens, i + 1);
			break;
		}
	...
	previousToken() {
		...
		for (let i = 1; i < tokens.length; i++) {
			...
			previousToken = this.previousNotIgnoredToken(tokens, i - 1);
			break;
		}
