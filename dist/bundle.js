...
var isInToken = function isInToken(_ref) {
	var line = _ref.line,
	    column = _ref.column;
	return function (token) {
		return token.loc.start.line <= line && token.loc.start.column <= column && token.loc.end.line >= line && token.loc.end.column >= column;
	};
};

var isAfterToken = function isAfterToken(_ref2) {
	var line = _ref2.line,
	    column = _ref2.column;
	return function (token) {
		return token.loc.end.line < line || token.loc.end.line === line && token.loc.end.column < column;
	};
};

var isBeforeToken = function isBeforeToken(_ref3) {
	var line = _ref3.line,
	    column = _ref3.column;
	return function (token) {
		return token.loc.start.line > line || token.loc.start.line === line && token.loc.start.column > column;
	};
};

function toBabelPoint(atomPoint) {
	return {
...
var pattern = function pattern(state) {
	return state.pattern;
};
var results = function results(state) {
	return state.results;
};
var editor = function editor(state) {
	return state.editor;
};
var visible = function visible(state) {
	return state.visible;
};
var tokenIndex = function tokenIndex(state) {
	return state.tokenIndex;
};
var searchEditor = function searchEditor(state) {
	return state.searchEditor;
};

var searchStatus = createSelector(pattern, results, function (pattern, results) {
...
var header = createSelector(searchStatus, results, pattern, function (status, results, pattern) {
	if (status === 'NO_SEARCH') {
...
var searchLabel = createSelector(searchStatus, tokenIndex, results, function (status, index, results) {
	if (status === 'NO_SEARCH') {
		return '';
...
var isFocused = function isFocused(state) {
	return searchEditor(state) && searchEditor(state).classList.contains('is-focused');
};

var selectedToken = createSelector(results, tokenIndex, function (results, index) {
...
var FuzzyTokenWrapper = function FuzzyTokenWrapper(_ref) {
	var visible$$1 = _ref.visible;
	return visible$$1 ? h(EnhancedFuzzyToken, null) : null;
};

var FuzzyView = connect(function (state) {
	return {
...
var FuzzyToken = function (_Component) {
	...
	function FuzzyToken(props) {
		...
		var _this = possibleConstructorReturn$1(this, (FuzzyToken.__proto__ || Object.getPrototypeOf(FuzzyToken)).call(this, props));
		...
		_this.bufferDisposable = null;
		_this.workspaceDisposable = null;
		return _this;
	}

	createClass(FuzzyToken, [{
		...
		value: function componentDidMount() {
			...
			var model = this.searchEditor.getModel();
			model.setPlaceholderText('Fuzzy search in this buffer');

			var buffer = this.searchEditor.getModel().getBuffer();
			this.bufferDisposable = buffer.onDidChange(function () {
				_this2.search();
			});
			this.workspaceDisposable = atom.workspace.onDidStopChangingActivePaneItem(function (editor$$1) {
				if (editor$$1.getBuffer) {
					_this2.props.setEditor(editor$$1);
					_this2.search();
				} else {
					_this2.hide();
				}
		...
		value: function search() {
			var buffer = this.searchEditor.getModel().getBuffer();
			var pattern = buffer.getText();
			var text = this.props.editor.getBuffer().getText();
			...
			var tokenValues = tokens.map(function (t) {
				return t.value || t.type.label;
			}).map(String);
			var results$$1 = fuzzy.filter(pattern, tokenValues, {
				pre: '<',
				post: '>'
			}).map(function (t) {
				return tokens[t.index].loc;
			}).map(toAtomRange);
		...
		value: function hide() {
			this.props.hide();
			var view = atom.views.getView(this.props.editor);
			view.focus();
		}
	}, {
		key: 'next',
		...
		value: function render$$1() {
			...
			var _props = this.props,
			    header$$1 = _props.header,
			    searchLabel$$1 = _props.searchLabel,
			    status = _props.status;


			var findAndReplaceClassNames = classnames('find-and-replace', {
			...
			return h(
				...
				h(
					...
					h(
						'div',
						{ className: findAndReplaceClassNames },
						h(
							'header',
							{ className: 'header' },
							h(
...
var EnhancedFuzzyToken = connect(function (state) {
	...
	return {
		next: function next(payload) {
			return dispatch({ type: 'NEXT', payload: payload });
		},
		previous: function previous(payload) {
			return dispatch({ type: 'PREVIOUS', payload: payload });
		},
		hide: function hide() {
			return dispatch({ type: 'HIDE' });
		},
		setSearchResults: function setSearchResults(payload) {
			return dispatch({ type: 'SET_SEARCH_RESULTS', payload: payload });
		},
		setSearchEditor: function setSearchEditor(payload) {
			return dispatch({ type: 'SET_SEARCH_EDITOR', payload: payload });
		},
		setEditor: function setEditor(payload) {
			return dispatch({ type: 'SET_EDITOR', payload: payload });
		}
	};
})(FuzzyToken);
...
var visible$1 = reducerCreator({
	SHOW: true,
	HIDE: false
})(false);

var editor$1 = reducerCreator({
	SHOW: returnPayload(),
	FOCUS: returnPayload(),
	SET_EDITOR: returnPayload()
})(null);

var searchEditor$1 = reducerCreator({
	SET_SEARCH_EDITOR: returnPayload()
})(null);

var results$1 = reducerCreator({
	HIDE: [],
	SET_SEARCH_RESULTS: returnPayload('results')
})([]);

var tokenIndex$1 = reducerCreator({
	HIDE: 0,
	NEXT: function NEXT(state, _ref2) {
		var resultsCount = _ref2.resultsCount;
	...
	PREVIOUS: function PREVIOUS(state, _ref3) {
		var resultsCount = _ref3.resultsCount;
	...
	SET_SEARCH_RESULTS: function SET_SEARCH_RESULTS(state, payload) {
		return state >= payload.results.length ? 0 : state;
	}
})(0);

var pattern$1 = reducerCreator({
	HIDE: '',
...
var config = {
	ignoredTokens: {
		...
		type: 'array',
		default: ['(', ')', '{', '}', '[', ']', ';', '.', '=', '=>', ':', 'jsxTagEnd', 'jsxTagStart', 'template']
	},
	ignoreCommas: {
		title: 'Ignore commas',
...
module.exports = {
	...
	onChange: function onChange() {
		...
		var editor$$1 = state.editor;
		if (searchStatus(state) === 'SEARCH_SUCCESS') {
			if (editor$$1) {
				editor$$1.setSelectedBufferRange(selectedToken(state));
			}
		}
	},
	...
	setup: function setup() {
		...
		atom.config.observe('token-navigation.ignoredTokens', function (value) {
			_this2.ignoredTokens = value;
		});

		atom.config.observe('token-navigation.ignoreCommas', function (value) {
			_this2.ignoreCommas = value;
		});
	...
	shouldIgnore: function shouldIgnore(token, selectedRange) {
		if (!token) {
			return true;
		}
		...
		var atomRange = toAtomRange(token.loc);
	...
	show: function show() {
		var editor$$1 = atom.workspace.getActiveTextEditor();
		this.store.dispatch({ type: 'SHOW', payload: editor$$1 });
	},
	focus: function focus() {
		var editor$$1 = atom.workspace.getActiveTextEditor();
		this.store.dispatch({ type: 'FOCUS', payload: editor$$1 });
		// Side effect: find a better solution
	...
	hide: function hide() {
		...
		var editor$$1 = atom.workspace.getActiveTextEditor();
		var view = atom.views.getView(editor$$1);
		view.focus();
	},
	nextNotIgnoredToken: function nextNotIgnoredToken(tokens, index$$1, selectedRange) {
	...
	previousNotIgnoredToken: function previousNotIgnoredToken(tokens, index$$1, selectedRange) {
	...
	multiNextToken: function multiNextToken() {
		var _this3 = this;

		var editor$$1 = atom.workspace.getActiveTextEditor();

		var buffer = editor$$1.getBuffer();
		var text = buffer.getText();
		var tokens = parse(text);
		var cursors = editor$$1.cursors;
		var selectedRanges = editor$$1.getSelectedBufferRanges();
		var ranges = cursors.map(function (cursor, index$$1) {
			return _this3.nextToken(editor$$1, tokens, cursor, selectedRanges[index$$1]);
		}).filter(function (x) {
			return x;
		});
		if (!ranges.length) {
		...
		editor$$1.setSelectedBufferRanges(ranges);
	},
	multiPreviousToken: function multiPreviousToken() {
		var _this4 = this;
		...
		var selectedRanges = editor$$1.getSelectedBufferRanges();
		var ranges = cursors.map(function (cursor, index$$1) {
			return _this4.previousToken(editor$$1, tokens, cursor, selectedRanges[index$$1]);
		}).filter(function (x) {
			return x;
		});
		if (!ranges.length) {
	...
	nextToken: function nextToken(editor$$1, tokens, cursor, selectedRange) {
		var _toBabelPoint = toBabelPoint(cursor.getBufferPosition()),
		    line = _toBabelPoint.line,
		    column = _toBabelPoint.column;

		var isInside = isInToken({ line: line, column: column });
		var isBefore = isBeforeToken({ line: line, column: column });
		var isAfter = isAfterToken({ line: line, column: column });
		...
		for (var i = 0; i < tokens.length - 1; i++) {
			...
			if (isInside(token)) {
				var nextValidToken = this.nextNotIgnoredToken(tokens, i, selectedRange);
				if (!nextValidToken) {
					return;
				}
			...
			if (isAfter(token) && isBefore(nextToken)) {
				var _nextValidToken = this.nextNotIgnoredToken(tokens, i + 1, selectedRange);
				if (!_nextValidToken) {
					return;
				}
				return toAtomRange(_nextValidToken.loc);
			}
		}
	},
	previousToken: function previousToken(editor$$1, tokens, cursor, selectedRange) {
		var _toBabelPoint2 = toBabelPoint(cursor.getBufferPosition()),
		    line = _toBabelPoint2.line,
		    column = _toBabelPoint2.column;
		...
		for (var i = 1; i < tokens.length; i++) {
			...
			if (isInside(token)) {
				var previousValidToken = this.previousNotIgnoredToken(tokens, i, selectedRange);
			...
			if (isBefore(token) && isAfter(previousToken)) {
				var _previousValidToken = this.previousNotIgnoredToken(tokens, i - 1, selectedRange);
				if (!_previousValidToken) {
					return;
				}
				return toAtomRange(_previousValidToken.loc);
			}
		}
	}
};
