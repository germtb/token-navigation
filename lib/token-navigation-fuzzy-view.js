...
const FuzzyTokenWrapper = ({ visible }) => {
	return visible ? <EnhancedFuzzyToken /> : null;
}

const EnhancedFuzzyTokenWrapper = connect(state => ({
...
class FuzzyToken extends React.PureComponent {
	...
	componentDidMount() {
		...
		this.workspaceDisposable = atom.workspace.onDidStopChangingActivePaneItem(editor => {
			if (editor.getBuffer) {
				...
				element.className = 'bottom';
					...
					<atom-panel ref={ element => {
						...
						<div className={ findAndReplaceClassNames } >
							...
							<section style={{ justifyContent: 'center' }} className='input-block find-container'>
...
const EnhancedFuzzyToken = connect(state => ({
	header: header(state),
	...
	setSearchEditor: payload => dispatch({ type: 'SET_SEARCH_EDITOR', payload }),
	setEditor: payload => dispatch({ type: 'SET_EDITOR', payload })
}))(FuzzyToken);

export function initReact(store) {
	const rootElement = document.querySelector('atom-workspace-axis.vertical atom-pane-container');
	const reactRoot = document.createElement('div');
	rootElement.insertAdjacentElement('afterend', reactRoot);
	ReactDOM.render(<Provider store={ store }>
		<EnhancedFuzzyTokenWrapper />
	</Provider>, reactRoot);
}
