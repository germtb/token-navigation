...
export default {
	...
	shouldIgnore(token, selectedRange) {
		...
		const atomRange = toAtomRange(token.loc.start, token.loc.end);
	...
	nextToken(editor, tokens, cursor, selectedRange) {
		...
		for (let i = 0; i < tokens.length - 1; i++) {
			...
			if (isInside(token)) {
				...
				return toAtomRange(nextValidToken.loc.start, nextValidToken.loc.end);
			}
	...
	previousToken(editor, tokens, cursor, selectedRange) {
		...
		for (let i = 1; i < tokens.length; i++) {
			...
			if (isInside(token)) {
				...
				return toAtomRange(previousValidToken.loc.start, previousValidToken.loc.end);
			}
